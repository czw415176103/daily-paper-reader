...
title_zh: BoolXLLM：基于大语言模型的布尔模型可解释性辅助框架
authors: "Du Cheng, Serdar Kadioglu, Xin Wang"
date: 2026-05-12
pdf: "https://arxiv.org/pdf/2605.12139v1"
tags: ["query:q7"]
score: 8.5
evidence: LLM辅助的布尔规则学习可解释性
tldr: 本研究针对基于布尔规则的可解释机器学习模型在实践中难以被非技术用户理解的问题，提出了BoolXLLM混合框架。该框架将大语言模型集成到布尔规则学习的端到端流程中，在特征选择、阈值推荐和规则解释三个阶段引入大语言模型的辅助，从而将形式化的忠实解释转化为人类可理解的叙述。这项工作结合了符号推理与基于语言的模型，旨在构建既理论扎实又对非专家友好的可解释AI系统。
source: arxiv
...
motivation: 旨在解决基于规则的布尔模型在实践中面临的两个主要挑战：选择有语义意义的特征，以及将形式逻辑规则转化为易于理解的自然语言解释。
method: 该方法在布尔规则学习的三个关键阶段（特征选择、阈值推荐、规则压缩与解释）中集成大语言模型，以增强可解释性。
result: 早期实证结果表明，大语言模型辅助的流程在保持竞争力的预测性能的同时，提高了模型的可解释性。
conclusion: 该研究展示了将符号推理与基于语言的大模型相结合，构建以人为本的可解释AI系统的潜力。
---

## 摘要
可解释机器学习旨在提供透明的模型，其决策过程能够易于人类理解。基于规则的方法（如表达性布尔公式BoolXAI）的最新进展，为模型行为提供了忠实且紧凑的表示。然而，对于非技术利益相关者而言，实践中仍存在主要挑战：（i）选择具有语义意义的特征，以及（ii）将形式逻辑规则转化为易于理解的解释。

本研究提出BoolXLLM，作为一个混合框架，将大语言模型集成到布尔规则学习的端到端流程中。我们在三个关键阶段用大语言模型增强表达性布尔规则分类器BoolXAI：（1）特征选择阶段，大语言模型指导识别领域相关变量；（2）阈值推荐阶段，大语言模型为数值特征提出具有语义意义的离散化策略；（3）规则压缩与解释阶段，将布尔规则转化为全局和局部层面的自然语言解释。

这种集成将形式化、忠实的解释与人类可理解的叙述相连接，从而构建一个既理论扎实又对非专家可访问的可解释人工智能系统。初步实证结果表明，大语言模型辅助的流程在保持竞争力的预测性能的同时提升了可解释性。我们的工作凸显了将符号推理与基于语言的模型相结合以实现以人为本的可解释性的前景。
